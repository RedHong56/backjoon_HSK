...
vector<vector<int>> solution(int n) {
  ...
  while (start_row <= end_row && start_col <= end_col) { // 이게 킥

    for (int i = start_col; i <= end_col; ++i) { //[,0] [,1] [,2] [,3] / [+,0] [+,1] [+,2] [+,3] 
      snail_array[start_row][i] = num++;
    }
    ++start_row;

    // 가장 왼쪽  아래부분 에서 가장 오른쪽 바로 직전 까지 채우기
    for (int i = start_row; i <= end_row; ++i) { //[1,] [2,] [3,]  / [1,-] [2,-] [3,-] 
      snail_array[i][end_col] = num++;
    }
    --end_col;

    // 가장 오른쪽 아래부분 에서 가장 위 바로 직전 까지 채우기
    if (start_row <= end_row) { // [3, 2], [3, 1]
